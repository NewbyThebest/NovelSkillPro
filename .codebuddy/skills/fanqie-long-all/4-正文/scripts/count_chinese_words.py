...
def count_single_chapter(chapter_num, chapter_dir):
    ...
    if words < 2000:
        shortage = 2000 - words
        print(f"⚠️  字数不足：还需 {shortage} 字才能达到最低标准 (2000字)")
    elif words > 4000:
        excess = words - 4000
        print(f"✅ 字数充足：超出建议上限 {excess} 字 (可接受)")
    else:
        print(f"✅ 字数达标：符合标准范围 (2000-4000字)")
...
def count_all_chapters(chapter_dir):
    ...
    average_words = total_words // chapter_count if chapter_count else 0
    print(f"章节数: {chapter_count} 章")
    ...
    print()
    print("=== 字数不足章节 (< 2000 字) ===")
    insufficient = [(i, w) for i, w in chapter_stats if 0 < w < 2000]

    if insufficient:
        for i, w in insufficient:
            print(f"⚠️  第{i:3d}章: {w:5d} 字 (不足 {2000-w} 字)")
    else:
        print("✅ 所有章节字数均达标")
...
def main():
    cwd = os.getcwd()
    default_chapter_dir = os.path.join(cwd, "4-正文")
    chapter_dir = default_chapter_dir if os.path.isdir(default_chapter_dir) else cwd
